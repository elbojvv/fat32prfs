 ...
 */
int fat_search_long_prfs(struct inode *inode, const unsigned char *name,
		    int name_len, struct fat_slot_info *sinfo)
{
	...
	//dbg printk(KERN_INFO "fat_search_long_prfs function...\n");
...
EXPORT_SYMBOL_GPL(fat_search_long_prfs);
			 ...
			 struct fat_ioctl_filldir_callback *both)
{
	...
	//dbg printk(KERN_INFO "__fat_readdir function...\n");
...
{
	//dbg printk(KERN_INFO "fat_readdir function...\n");
			  ...
			  unsigned long arg)
{
	...
	//dbg printk(KERN_INFO "fat_dir_ioctl function...\n");
				 ...
				 unsigned long arg)
{
	...
	//dbg printk(KERN_INFO "fat_compat_dir_ioctl function...\n");
 ...
 */
int fat_get_dotdot_entry_prfs(struct inode *dir, struct buffer_head **bh,
			 struct msdos_dir_entry **de)
{
	loff_t offset = 0;

	//dbg printk(KERN_INFO "fat_get_dotdot_entry_prfs function...\n");
...
EXPORT_SYMBOL_GPL(fat_get_dotdot_entry_prfs);

/* See if directory is empty */
int fat_dir_empty_prfs(struct inode *dir)
{
	struct buffer_head *bh;
	struct msdos_dir_entry *de;
	loff_t cpos;
	...
	//dbg printk(KERN_INFO "fat_dir_empty_prfs function...\n");
...
EXPORT_SYMBOL_GPL(fat_dir_empty_prfs);
 ...
 */
int fat_scan_prfs(struct inode *dir, const unsigned char *name,
...
EXPORT_SYMBOL_GPL(fat_scan_prfs);
...
int fat_remove_entries_prfs(struct inode *dir, struct fat_slot_info *sinfo)
{
	struct super_block *sb = dir->i_sb;
	struct msdos_dir_entry *de;
	struct buffer_head *bh;
	...
	//dbg printk(KERN_INFO "fat_remove_entries_prfs function...\n");
	...
	fat_truncate_time_prfs(dir, NULL, S_ATIME|S_MTIME);
	if (IS_DIRSYNC(dir))
...
EXPORT_SYMBOL_GPL(fat_remove_entries_prfs);
...
int fat_alloc_new_dir_prfs(struct inode *dir, struct timespec64 *ts)
{
	struct super_block *sb = dir->i_sb;
	struct msdos_sb_info *sbi = MSDOS_SB(sb);
	...
	//dbg printk(KERN_INFO "fat_alloc_new_dir_prfs function...\n");
	...
	fat_time_unix2fat_prfs(sbi, ts, &time, &date, &time_cs);
...
EXPORT_SYMBOL_GPL(fat_alloc_new_dir_prfs);
...
error_nomem:
	for (i = 0; i < n; i++)
		bforget(bhs[i]);
	fat_free_clusters_prfs(dir, cluster[0]);
error:
	return err;
}

int fat_add_entries_prfs(struct inode *dir, void *slots, int nr_slots,
		    struct fat_slot_info *sinfo)
{
	struct super_block *sb = dir->i_sb;
	struct msdos_sb_info *sbi = MSDOS_SB(sb);
	...
	//dbg printk(KERN_INFO "fat_add_entries_prfs function...\n");
...
EXPORT_SYMBOL_GPL(fat_add_entries_prfs);
