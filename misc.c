...
void __fat_fs_error_prfs(struct super_block *sb, int report, const char *fmt, ...)
{
	...
	//dbg printk(KERN_INFO "__fat_fs_error_prfs function...\n");
...
EXPORT_SYMBOL_GPL(__fat_fs_error_prfs);
...
void fat_time_fat2unix_prfs(struct msdos_sb_info *sbi, struct timespec64 *ts,
		       __le16 __time, __le16 __date, u8 time_cs)
{
	...
	//dbg printk(KERN_INFO "fat_time_fat2unix_prfs function...\n");
...
EXPORT_SYMBOL_GPL(fat_time_fat2unix_prfs);

/* Convert linear UNIX date to a FAT time/date pair. */
void fat_time_unix2fat_prfs(struct msdos_sb_info *sbi, struct timespec64 *ts,
		       __le16 *time, __le16 *date, u8 *time_cs)
{
	struct tm tm;
	time64_to_tm(ts->tv_sec, -fat_tz_offset(sbi), &tm);

	//dbg printk(KERN_INFO "fat_time_unix2fat_prfs function...\n");
...
EXPORT_SYMBOL_GPL(fat_time_unix2fat_prfs);
 ...
 */
int fat_truncate_time_prfs(struct inode *inode, struct timespec64 *now, int flags)
{
	struct msdos_sb_info *sbi = MSDOS_SB(inode->i_sb);
	struct timespec64 ts;

	//dbg printk(KERN_INFO "fat_truncate_time_prfs function...\n");
...
EXPORT_SYMBOL_GPL(fat_truncate_time_prfs);

int fat_update_time_prfs(struct inode *inode, struct timespec64 *now, int flags)
{
	int dirty_flags = 0;

	//dbg printk(KERN_INFO "fat_update_time_prfs function...\n");

	if (inode->i_ino == MSDOS_ROOT_INO)
		return 0;

	if (flags & (S_ATIME | S_CTIME | S_MTIME)) {
		fat_truncate_time_prfs(inode, now, flags);
...
EXPORT_SYMBOL_GPL(fat_update_time_prfs);
