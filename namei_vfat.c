...
static int vfat_find_form(struct inode *dir, unsigned char *name)
{
	...
	int err = fat_scan_prfs(dir, name, &sinfo);
	if (err)
		return -ENOENT;
	brelse(sinfo.bh);
	return 0;
			  ...
			  struct fat_slot_info *sinfo)
{
	...
	err = fat_add_entries_prfs(dir, slots, nr_slots, sinfo);
	if (err)
		goto cleanup;
		     ...
		     struct fat_slot_info *sinfo)
{
	...
	if (len == 0)
		return -ENOENT;
	return fat_search_long_prfs(dir, qname->name, len, sinfo);
}
		       ...
		       struct dentry *dentry, umode_t mode, bool excl)
{
	...
	printk(KERN_INFO "vfat_create function: %s\n", dentry->d_iname);
...
static int vfat_rmdir(struct inode *dir, struct dentry *dentry)
{
	...
	clear_nlink(inode);
	fat_truncate_time_prfs(inode, NULL, S_ATIME|S_MTIME);
...
{
	if (S_ISDIR(inode->i_mode)) {
		if (fat_get_dotdot_entry_prfs(inode, bh, de))
			return -EIO;
	}
	return 0;
}
...
{
	if (IS_DIRSYNC(dir))
		return fat_sync_inode_prfs(inode);
			       ...
			       loff_t old_i_pos, loff_t new_i_pos)
{
	...
	fat_detach_prfs(new_inode);
	fat_attach_prfs(old_inode, new_i_pos);
	fat_attach_prfs(new_inode, old_i_pos);
}
...
{
	return fat_fill_super_prfs(sb, data, silent, 1, setup);
}

static struct dentry *vfatprfs_mount(struct file_system_type *fs_type,
...
static struct file_system_type vfat_fs_type = {
	.owner		= THIS_MODULE,
	.name		= "vfatprfs",
	.mount		= vfatprfs_mount,
	.kill_sb	= kill_block_super,
	.fs_flags	= FS_REQUIRES_DEV | FS_ALLOW_IDMAP,
};
MODULE_ALIAS_FS("vfatprfs");
