		      ...
		      struct fat_slot_info *sinfo)
{
	...
	err = fat_scan_prfs(dir, msdos_name, sinfo);
				   ...
				   unsigned int flags)
{
	...
	case -ENOENT:
		...
		break;
	case 0:
		inode = fat_build_inode_prfs(sb, sinfo.de, sinfo.i_pos);
		brelse(sinfo.bh);
		break;
	default:
		inode = ERR_PTR(err);
			   ...
			   struct timespec64 *ts, struct fat_slot_info *sinfo)
{
	...
	if (is_hid)
		de.attr |= ATTR_HIDDEN;
	de.lcase = 0;
	fat_time_unix2fat_prfs(sbi, ts, &time, &date, NULL);
	de.cdate = de.adate = 0;
	de.ctime = 0;
	de.ctime_cs = 0;
	de.time = time;
	...
	err = fat_add_entries_prfs(dir, &de, 1, sinfo);
	if (err)
		return err;

	fat_truncate_time_prfs(dir, ts, S_CTIME|S_MTIME);
	if (IS_DIRSYNC(dir))
		(void)fat_sync_inode_prfs(dir);
	else
			...
			struct dentry *dentry, umode_t mode, bool excl)
{
	...
	if (!fat_scan_prfs(dir, msdos_name, &sinfo)) {
		brelse(sinfo.bh);
		err = -EINVAL;
		goto out;
	}
	...
	fat_truncate_time_prfs(inode, &ts, S_ATIME|S_CTIME|S_MTIME);
...
out:
	mutex_unlock(&MSDOS_SB(sb)->s_lock);
	if (!err)
		err = fat_flush_inodes_prfs(sb, dir, inode);
	return err;
}
...
static int msdos_rmdir(struct inode *dir, struct dentry *dentry)
{
	...
	err = fat_remove_entries_prfs(dir, &sinfo);	/* and releases bh */
	if (err)
		goto out;
	drop_nlink(dir);

	clear_nlink(inode);
	fat_truncate_time_prfs(inode, NULL, S_CTIME);
		       ...
		       struct dentry *dentry, umode_t mode)
{
	...
	ts = current_time(dir);
	cluster = fat_alloc_new_dir_prfs(dir, &ts);
	if (cluster < 0) {
		err = cluster;
		goto out;
	}
	...
	mutex_unlock(&MSDOS_SB(sb)->s_lock);
	fat_flush_inodes_prfs(sb, dir, inode);
	return 0;

out_free:
	fat_free_clusters_prfs(dir, cluster);
out:
	mutex_unlock(&MSDOS_SB(sb)->s_lock);
	return err;
}
			   ...
			   struct dentry *new_dentry, int is_hid)
{
	...
	err = fat_scan_prfs(old_dir, old_name, &old_sinfo);
	if (err) {
		err = -EIO;
		goto out;
	}
	...
	if (update_dotdot) {
		if (fat_get_dotdot_entry_prfs(old_inode, &dotdot_bh, &dotdot_de)) {
			err = -EIO;
			goto out;
		}
	}
	...
	err = fat_scan_prfs(new_dir, new_name, &sinfo);
	if (!err) {
		if (!new_inode) {
			...
			if (IS_DIRSYNC(old_dir)) {
				err = fat_sync_inode_prfs(old_inode);
				if (err) {
			...
			fat_truncate_time_prfs(old_dir, NULL, S_CTIME|S_MTIME);
			if (IS_DIRSYNC(old_dir))
				(void)fat_sync_inode_prfs(old_dir);
			else
	...
	if (new_inode) {
		if (err)
			goto out;
		if (is_dir) {
			err = fat_dir_empty_prfs(new_inode);
			if (err)
				goto out;
		}
		new_i_pos = MSDOS_I(new_inode)->i_pos;
		fat_detach_prfs(new_inode);
	} else {
	...
	fat_detach_prfs(old_inode);
	fat_attach_prfs(old_inode, new_i_pos);
	if (is_hid)
		MSDOS_I(old_inode)->i_attrs |= ATTR_HIDDEN;
	else
		MSDOS_I(old_inode)->i_attrs &= ~ATTR_HIDDEN;
	...
	err = fat_remove_entries_prfs(old_dir, &old_sinfo);	/* and releases bh */
	old_sinfo.bh = NULL;
	if (err)
		goto error_dotdot;
	inode_inc_iversion(old_dir);
	fat_truncate_time_prfs(old_dir, &ts, S_CTIME|S_MTIME);
	if (IS_DIRSYNC(old_dir))
		(void)fat_sync_inode_prfs(old_dir);
	else
		mark_inode_dirty(old_dir);
	...
	if (new_inode) {
		...
		if (is_dir)
			drop_nlink(new_inode);
		fat_truncate_time_prfs(new_inode, &ts, S_CTIME);
	}
out:
	brelse(sinfo.bh);
	brelse(dotdot_bh);
...
error_inode:
	fat_detach_prfs(old_inode);
	fat_attach_prfs(old_inode, old_sinfo.i_pos);
	MSDOS_I(old_inode)->i_attrs = old_attrs;
	if (new_inode) {
		fat_attach_prfs(new_inode, new_i_pos);
		if (corrupt)
			corrupt |= fat_sync_inode_prfs(new_inode);
	} else {
		/*
		 * If new entry was not sharing the data cluster, it
		 * shouldn't be serious corruption.
		 */
		int err2 = fat_remove_entries_prfs(new_dir, &sinfo);
		if (corrupt)
			corrupt |= err2;
		sinfo.bh = NULL;
	}
...
out:
	mutex_unlock(&MSDOS_SB(sb)->s_lock);
	if (!err)
		err = fat_flush_inodes_prfs(sb, old_dir, new_dir);
	return err;
}
...
{
	return fat_fill_super_prfs(sb, data, silent, 0, setup);
}

static struct dentry *msdosprfs_mount(struct file_system_type *fs_type,
...
static struct file_system_type msdos_fs_type = {
	.owner		= THIS_MODULE,
	.name		= "msdosprfs",
	.mount		= msdosprfs_mount,
	.kill_sb	= kill_block_super,
	.fs_flags	= FS_REQUIRES_DEV | FS_ALLOW_IDMAP,
};
MODULE_ALIAS_FS("msdosprfs");
